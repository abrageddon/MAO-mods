...
gldelf_l1om_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf_l1om_write_build_id_section (bfd *abfd)
{
...
gldelf_l1om_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf_l1om_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf_l1om_write_build_id_section;
...
OUTPUT_ARCH(l1om)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/usr/local/x86_64-linux/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
