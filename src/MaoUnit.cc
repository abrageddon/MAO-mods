...
MaoUnit::~MaoUnit() {
	...
	for (std::vector<SubSection *>::iterator iter = sub_sections_.begin();
...
public:
	...
	virtual void Invoke(FILE *outfile) {
		fprintf(
				outfile,
				"***   generating asm for entry [%d], line: %d\n",
				entry_->id(),
				entry_->line_number());
	}
...
void MaoUnit::PrintIR(
		bool print_entries,
		bool print_sections,
		bool print_subsections,
		bool print_functions) const {
...
void MaoUnit::PrintIR(
	...
	if (print_entries) {
		// Print the entries
		for (std::vector<SubSection *>::const_iterator iter =
				sub_sections_.begin(); iter != sub_sections_.end(); ++iter) {
			SubSection *ss = *iter;
			for (EntryIterator e_iter = ss->EntryBegin();
					e_iter != ss->EntryEnd(); ++e_iter) {
				MaoEntry *e = *e_iter;
				fprintf(
						out,
						"[%5d][%c][%14s][%10s] ",
						e->id(),
						e->GetDescriptiveChar(),
						FunctionName(e),
						SectionName(e));
	...
	if (print_sections) {
		...
		for (ConstSectionIterator iter = ConstSectionBegin();
			...
			for (std::vector<SubSectionID>::const_iterator ss_iter =
					subsectionIDs.begin(); ss_iter != subsectionIDs.end();
					++ss_iter) {
...
SubSection *MaoUnit::AddNewSubSection(
		const char *section_name,
...
LabelEntry *MaoUnit::GetLabelEntry(const char *label_name) const {
	std::map<const char *, LabelEntry *>::const_iterator iter = labels_.find(
			label_name);
	if (iter == labels_.end()) {
		return NULL;
	} else {
...
static struct insn_template FindTemplate(
		const char *opcode,
...
InstructionEntry *MaoUnit::CreateInstruction(
		i386_insn *instruction,
		Function *function) {
	enum flag_code flag;
	switch (arch_) {
	case X86_64:
...
InstructionEntry *MaoUnit::CreatePrefetch(
		Function *function,
		int type,
...
InstructionEntry *MaoUnit::CreateUncondJump(
		LabelEntry *label,
		Function *function) {
	InstructionEntry *e = CreateInstruction("jmp", 0xeb, function);
	expressionS *disp_expression = static_cast<expressionS *>(xmalloc(
			sizeof(expressionS)));
	symbolS *symbolP;
...
InstructionEntry *MaoUnit::CreateIncFromOperand(
		Function *function,
...
InstructionEntry *MaoUnit::CreateDecFromOperand(
		Function *function,
		InstructionEntry *insn2,
		int op2) {
...
LabelEntry *MaoUnit::CreateLabel(
...
DirectiveEntry *MaoUnit::CreateDirective(
		...
		Function *function,
		SubSection *subsection) {
	DirectiveEntry *directive = new DirectiveEntry(op, operands, 0, NULL, this);
	// next free ID for the entry
	EntryID entry_index = entry_vector_.size();
...
bool MaoUnit::AddEntry(MaoEntry *entry, bool create_default_section) {
	...
	case MaoEntry::LABEL:
		...
		MAO_ASSERT(
				labels_.insert(std::make_pair(label_entry->name(), label_entry)).second);
		symbol = symbol_table_.FindOrCreateAndFind(
				label_entry->name(),
		...
		case DirectiveEntry::SET: {
			...
			if (op_expr->type == DirectiveEntry::EXPRESSION) {
				...
				if (op_as_expr->X_op == O_symbol) {
					Symbol *op_symbol_2 = symbol_table_.Find(
							S_GET_NAME(op_as_expr->X_add_symbol));
...
bool MaoUnit::AddCommSymbol(
	...
	if (!symbol_table_.Exists(name)) {
		...
		Section *section =
				current_subsection_ ? (current_subsection_->section()) : NULL;
		symbol = symbol_table_.Add(
...
void MaoUnit::FindFunctions(bool create_anonymous) {
			...
			while (entry_tail->next()) {
				...
				if (entry_tail->Type() == MaoEntry::DIRECTIVE) {
					...
					if (directive_entry->op() == DirectiveEntry::SIZE) {
						...
						const char *size_symbol_name = S_GET_NAME(
								d_op->data.sym);
	...
	if (create_anonymous == true) {
		...
		for (SectionIterator iter = SectionBegin(); iter != SectionEnd();
				++iter) {
			// Make sure the section exists in bfd, and it nos created
			// created temporarily inside mao.
			Section *section = *iter;
			if (bfd_get_section_by_name(stdoutput, section->name().c_str())
					!= NULL && section->name() != ".eh_frame"
					&& section->name() != ".bss" && section->name() != ".rodata"
					&& section->name() != ".data") {
...
void MaoUnit::DeleteEntry(MaoEntry *entry) {
	...
	if (InFunction(entry)) {
		...
		if (MaoRelaxer::HasSizeMap(section)) {
			std::map<MaoEntry *, int> *sizes = MaoRelaxer::GetSizeMap(
					this,
					section);
...
void MaoUnit::PushSubSection() {
	MAO_ASSERT(current_subsection_);
	subsection_stack_.push(
			std::make_pair(current_subsection_, prev_subsection_));
...
void MaoUnit::PopSubSection(int line_number) {
	...
	operands.push_back(new DirectiveEntry::Operand(ss->name().c_str()));
	DirectiveEntry *directive = new DirectiveEntry(
			DirectiveEntry::SECTION,
			operands,
			line_number,
			NULL,
			this);
	...
	if (ss->number() != 0) {
		...
		DirectiveEntry *ss_directive = new DirectiveEntry(
				DirectiveEntry::SUBSECTION,
				ss_operand,
				line_number,
				NULL,
				this);
...
void MaoUnit::SetPreviousSubSection(int line_number) {
	...
	DirectiveEntry::OperandVector operands;
	operands.push_back(
			new DirectiveEntry::Operand(prev_subsection_->name().c_str()));
	DirectiveEntry *directive = new DirectiveEntry(
			DirectiveEntry::SECTION,
			operands,
			line_number,
