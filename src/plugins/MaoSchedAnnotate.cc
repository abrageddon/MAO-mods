...
// Scheduler used to denote which lines are swapable with which other lines
//
#include "Mao.h"
...
// Options
// --------------------------------------------------------------------
MAO_DEFINE_OPTIONS(SCHEDA, "Annotates scheduled instructions at the assembly level", \
                   5) {
...
class SchedAnnotatePass : public MaoFunctionPass {
 public:
   ...
   * appear consecutively in code. For instance a lock should always precede the
  ...
  SchedAnnotatePass(MaoOptionMap *options, MaoUnit *mao, Function *func)
    : MaoFunctionPass("SCHEDA", options, mao, func) {
    ...
    FORALL_CFG_BB(cfg, bb_iterator) {
      ...
      if (dag != NULL) {
        ...
        for (int i = 0; i < dag->NodeCount(); i++) {
          Trace(1, "***%s: %d", insn_str_[i].c_str(), dependence_heights[i]);
        }
...
void SchedAnnotatePass::FindBBsInStraightLineLoops() {
...
void SchedAnnotatePass::FindBBsInStraightLineLoops(SimpleLoop *loop) {
...
MaoEntry* SchedAnnotatePass::Schedule(DependenceDag *dag,
                                  int *dependence_heights,
                                  MaoEntry *head,
                                  MaoEntry *last_entry) {
  ...
  while (!ready->empty()) {
    ...
    //SNEISIUS grp has changed; new schedule requirements
    if (newSucc){
      grpNum++;
    }
  }
 ...
 */
void SchedAnnotatePass::ScheduleNode(int node_index, MaoEntry **head,
                                 MaoEntry **last) {
  SchedulerNode *node = entries_[node_index];
...
int SchedAnnotatePass::RemoveTallest(std::list<int> *list, int *heights) {
  ...
  Trace(1, "BEST: %d", best_node);
  return best_node;
}

//SNEISIUS
/*int SchedAnnotatePass::RemoveRandom(std::list<int> *list, int *heights) {
	int best_node = -1;
...
int *SchedAnnotatePass::ComputeDependenceHeights(DependenceDag *dag) {
  int *heights, *visited;
...
BitString SchedAnnotatePass::GetSrcRegisters(SchedulerNode *node) {
  BitString use_mask;
...
BitString SchedAnnotatePass::GetDestRegisters(SchedulerNode *node) {
  BitString def_mask;
  for (MaoEntry *entry = node->first; entry != node->last->next();
       entry = entry->next()) {
    if (entry->IsInstruction()) {
...
void SchedAnnotatePass::InitializeLastWriter(int *last_writer) {
  int insns_in_bb = 0;
...
int  SchedAnnotatePass::CreateSchedulerNodes(MaoEntry *head, BasicBlock *bb) {
  int retain_next = 0;
  MaoEntry *first = NULL;
...
SchedAnnotatePass::DependenceDag *SchedAnnotatePass::FormDependenceDag(BasicBlock *bb) {
...
bool SchedAnnotatePass::HasMemOperation(SchedulerNode *node) const {
  for (MaoEntry *entry = node->first; entry != node->last->next();
       entry = entry->next()) {
    if (entry->IsInstruction()) {
      InstructionEntry *insn = entry->AsInstruction();
...
bool SchedAnnotatePass::IsMemCFIDirective(MaoEntry *entry) const {
...
bool SchedAnnotatePass::IsMemOperation(InstructionEntry *entry) const {
    if (entry->IsCall())
      return true;
    if (entry->op() == OP_lea)
      return false;
...
bool SchedAnnotatePass::HasPredicateOperation(SchedulerNode *node) const {
  for (MaoEntry *entry = node->first; entry != node->last->next();
       entry = entry->next()) {
    if (entry->IsInstruction()) {
      InstructionEntry *insn = entry->AsInstruction();
...
bool SchedAnnotatePass::IsPredicateOperation(InstructionEntry *entry) const {
  switch (entry->op()) {
    case OP_cmovo:
    case OP_cmovno:
    case OP_cmovb:
...
bool SchedAnnotatePass::HasControlOperation(SchedulerNode *node) const {
  for (MaoEntry *entry = node->first; entry != node->last->next();
       entry = entry->next()) {
    if (entry->IsInstruction()) {
      InstructionEntry *insn = entry->AsInstruction();
...
bool SchedAnnotatePass::IsControlOperation(InstructionEntry *entry) const {
...
// not
bool SchedAnnotatePass::IsProfitable(Function *function) {
...
REGISTER_PLUGIN_FUNC_PASS("SCHEDA", SchedAnnotatePass)
}  // namespace
